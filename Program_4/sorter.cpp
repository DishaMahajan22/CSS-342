...
int Elapsed(const timeval &first, const timeval &end); 

// I used the lecture code or the Cerrano textbook psuedocode as the base logic for my code
...
{ 
 ...
 struct timeval first_time, end_time; 
 ...
 cout << "Time (microsecs): " << Elapsed(first_time, end_time) <<  endl; 
 return 0; 
} 
...
int Elapsed(const timeval &first, const timeval &end)  
{ 
 return (end.tv_sec - first.tv_sec) * 1000000  
  + (end.tv_usec - first.tv_usec); 
} 
